...
export const saveSupplierDataOffline = async (supplierData) => {
  try {
    ...
    // on verifie si le cache existe déjà
    // si oui on le parse c'est à dire on créé un object js existing, sinon on initialise un tableau vide
    const suppliers = existing ? JSON.parse(existing) : [];

    suppliers.push(newSupplier);
    // avant de stocké avec AsyncStorage, on le parse en JSON
    // et on le stocke dans le cache
